    ...
    \subsection{Phase 2}
        \begin{enumerate}
            ...
            \item 使用 \textbf{gcc -c} 命令对其进行编译，并使用 \textbf{objdump -d} 命令反汇编得到其机器码
                \begin{figure*}[htbp]
                    \includegraphics*[width = 10cm]{s2_1.png} \\
            ...
            \item 使用 \textbf{gdb} 调试 \textbf{ctarget} 程序，之前已经确定过 \textbf{getbuf()} 的地址，通过 \textbf{break} 命令在相应位置设置断点。
                ...
                \end{figure*}
            \item 运行程序，并使用 \textbf{p \$rsp} 查看栈帧指向的地址，将其减去 \textbf{0x18} 即是栈底的位置。
                \begin{figure*}[htbp]
                    \includegraphics*[width = 10cm]{s2_4.png}
                \end{figure*}
    ...
    \subsection{Phase 3}
        \begin{enumerate}
            ...
            \item \sout{本着能查资料就不动脑子的原则}，我打开了官方手册查看了提示，发现了如下几句话：
                \begin{mquote}
                    When functions hexmatch and strncmp are called, they push data onto the stack, overwritingportions of memory that held the buffer used by getbuf.
                \end{mquote}
                ...
                \end{figure*}
                \par 于是，我们需要将字符串放置在一个绝对安全的位置，回想一下，在程序正常运行的时候，哪个位置是绝对不会被覆盖的呢？俗话说得好，最安全的地方就是最安全的地方，也就是原来存放 \textbf{getbuf()} 返回地址的位置及其之上的位置(因为堆栈的存储是由高到低的)。
            \item 依据上述想法，我们重新编写 \textbf{Injected Code}，因为返回地址长度是 \textbf{8} 字节，所以我们将字符串放到 \textbf{\%rsp + 0x8} (此处 \%rsp 指的是栈顶)的位置。
                \begin{figure*}[htbp]
                    \includegraphics*[width = 10cm]{s3_5.png}
                \end{figure*}
